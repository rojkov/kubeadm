...
function get_path_kube_artifacts() {
    local artifacts_dir="${KUBE_ROOT}/bazel-bin/build"
    if [ ! -d ${artifacts_dir} ]; then
	echo "No Bazel build found. Exiting..."
	exit 1
    fi

    local rpath=$(realpath ${artifacts_dir})
    if [ ! -d ${rpath} ]; then
	echo "${rpath} doesn't exist. Exiting..."
	exit 1
    fi
...
BINARIES_DIR=$(get_path_kube_artifacts)
...
function create_machine_from_current_dir() {
    ...
    local virt_install_cmd="virt-install --import --name ${machine} --ram 2048 --vcpus 2 --disk $disk,format=qcow2,bus=virtio --disk ${ci_iso},device=cdrom --network bridge=virbr0,model=virtio --os-type=linux --noautoconsole --filesystem ${BINARIES_DIR},kubernetes_tag"
